...
public class TransformerProcessor {
    ...
    public static void main(String... args) {
        ...
        final OptionSpec<Void> ignoreInvalid = optionParser.accepts("ignore-invalid", "Prevents the exception being thrown when encountering invalid config files, should only be used for legacy data");
        ...
        processJar(inputJarPath, outputJarPath, atFilePaths, optionSet.has(ignoreInvalid));
    ...
    private static void processJar(final Path inputJar, final Path outputJarPath, final List<Path> atFilePaths, final boolean ignoreInvalid) {
        for (Path path : atFilePaths) {
            AccessTransformerEngine.INSTANCE.addResource(path, path.getFileName().toString(), ignoreInvalid);
