...
public interface SpriteResourceLoader {
   ...
   static SpriteResourceLoader create(Set<MetadataSectionType<?>> additionalMetadataSections) {
      return (spriteLocation, resource) -> {
         ...
         if (animationInfo.isPresent()) {
            ...
            if (!Mth.isMultipleOf(image.getWidth(), frameSize.width()) || !Mth.isMultipleOf(image.getHeight(), frameSize.height())) {
               LOGGER.error(
                  ...
                  new Object[]{spriteLocation, image.getWidth(), image.getHeight(), frameSize.width(), frameSize.height()}
               );
               image.close();
               return null;
            }
         ...
         SpriteContents contents = ForgeHooksClient.loadSpriteContents(spriteLocation, resource, frameSize, image, additionalMetadata);
         return contents != null ? contents : new SpriteContents(spriteLocation, frameSize, image, animationInfo, additionalMetadata, textureInfo);
      };
   }
