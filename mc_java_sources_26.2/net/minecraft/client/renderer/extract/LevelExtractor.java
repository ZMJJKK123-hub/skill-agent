...
public class LevelExtractor implements ResourceManagerReloadListener {
   ...
   private void extractVisibleBlockEntities(Camera camera, float deltaPartialTick, LevelRenderState levelRenderState, Frustum frustum) {
      Vec3 cameraPos = camera.position();
      double camX = cameraPos.x();
      double camY = cameraPos.y();
      double camZ = cameraPos.z();
      ...
      while (iteratorx.hasNext()) {
         ...
         if (blockEntity.isRemoved()) {
            iteratorx.remove();
         } else if (frustum.isVisible(blockEntity.getRenderBoundingBox())) {
         ...
         if (!(pos.distToCenterSqr(camX, camY, camZ) > 1024.0)) {
            ...
            if (progresses != null && !progresses.isEmpty()) {
               ...
               levelRenderState.blockBreakingRenderStates
                  .add(new BlockBreakingRenderState(pos, this.level.getBlockState(pos), progress, this.level.getModelDataManager().getAtOrEmpty(pos)));
            }
         }
      }
   }
   ...
   private void extractBlockOutline(Camera camera, LevelRenderState levelRenderState) {
      ...
      RenderHighlightEvent.Callback custom = ForgeHooksClient.onExtractBlockOutline(this, camera, levelRenderState, this.minecraft.hitResult);
      if (custom != null) {
         levelRenderState.blockOutlineRenderState = new BlockOutlineRenderState(BlockPos.ZERO, false, false, Shapes.empty(), null, null, null, custom);
      } else {
