...
public class ScreenEffectRenderer {
   ...
   public void submit(boolean isFirstPerson, boolean isSleeping, float partialTicks, SubmitNodeCollector submitNodeCollector, boolean hideGui) {
      ...
      if (isFirstPerson && !isSleeping) {
         ...
         BlockState blockState = overlay == null ? null : (BlockState)overlay.getLeft();
         if (blockState != null
            && ForgeHooksClient.renderBlockOverlay(
               player, poseStack, RenderBlockScreenEffectEvent.OverlayType.BLOCK, (BlockState)overlay.getLeft(), (BlockPos)overlay.getRight()
            )) {
         ...
         if (!this.minecraft.player.isSpectator()) {
            if (this.minecraft.player.isEyeInFluid(FluidTags.WATER)) {
               if (!ForgeHooksClient.renderWaterOverlay(player, poseStack)) {
                  submitWater(this.minecraft, poseStack, submitNodeCollector);
               }
            } else if (!player.getEyeInFluidType().isAir()) {
               IClientFluidTypeExtensions.of(player.getEyeInFluidType()).renderOverlay(this.minecraft, poseStack, submitNodeCollector);
            }

            if (this.minecraft.player.isOnFire()) {
               TextureAtlasSprite fireSprite = this.sprites.get(ModelBakery.FIRE_1);
               if (!ForgeHooksClient.renderFireOverlay(player, poseStack)) {
   ...
   private static @Nullable BlockState getViewBlockingState(Player player) {
      ...
      Pair<BlockState, BlockPos> ret = getOverlayBlock(player);
   ...
   private static @Nullable Pair<BlockState, BlockPos> getOverlayBlock(Player player) {
      ...
      for (int i = 0; i < 8; i++) {
         ...
         if (blockState.getRenderShape() != RenderShape.INVISIBLE && blockState.isViewBlocking(player.level(), testPos)) {
            return Pair.of(blockState, testPos.immutable());
         }
      }
   ...
   private static void submitWater(Minecraft minecraft, PoseStack poseStack, SubmitNodeCollector submitNodeCollector) {
      renderFluid(minecraft, poseStack, submitNodeCollector, UNDERWATER_LOCATION);
   }

   public static void renderFluid(Minecraft minecraft, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, Identifier texture) {
