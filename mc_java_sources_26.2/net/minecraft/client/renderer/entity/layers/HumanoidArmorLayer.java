...
public class HumanoidArmorLayer<S extends HumanoidRenderState, M extends HumanoidModel<S>, A extends HumanoidModel<S>> extends RenderLayer<S, M> {
   ...
   private void renderArmorPiece(
      ...
      if (equippable != null && shouldRender(equippable, slot)) {
         ...
         Model newModel = this.getArmorModel(state, slot, itemStack, model);
   ...
   protected Model getArmorModel(S state, EquipmentSlot slot, ItemStack itemStack, A _default) {
      return ForgeHooksClient.getArmorModel(state, itemStack, slot, _default);
   }
}
