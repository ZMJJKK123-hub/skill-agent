...
public class Hud {
   ...
   public Hud(Minecraft minecraft) {
      ...
      this.contextualInfoBars = ImmutableMap.of(
         ...
         (Supplier<ContextualBar>)() -> ContextualBar.EMPTY,
         Hud.ContextualInfo.EXPERIENCE,
         (Supplier<ContextualBar>)() -> new ExperienceBar(minecraft),
         Hud.ContextualInfo.LOCATOR,
         (Supplier<ContextualBar>)() -> new LocatorBar(minecraft),
         Hud.ContextualInfo.JUMPABLE_VEHICLE,
         (Supplier<ContextualBar>)() -> new JumpableVehicleBar(minecraft)
      );
      this.resetTitleTimes();
      ForgeLayeredDraw.init(this, minecraft);
   }
   ...
   public void extractRenderState(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
      ...
      if (!(this.minecraft.gui.screen() instanceof LevelLoadingScreen)) {
         if (Boolean.valueOf(true)) {
            ForgeLayeredDraw.extractRenderState(graphics, deltaTracker);
            return;
         }
   ...
   public void extractBossOverlay(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
   ...
   public void extractSubtitleOverlay(GuiGraphicsExtractor graphics, boolean deferRendering) {
      if (deferRendering) {
   ...
   public void extractCameraOverlays(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
   ...
   public void extractSleepOverlay(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
   ...
   public void extractOverlayMessage(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
   ...
   public void extractTitle(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
   ...
   public void extractChat(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
      if (this.minecraft.player != null && !this.chat.isChatFocused()) {
         ...
         graphics.nextStratum();
         ForgeHooksClient.onCustomizeChatEvent(graphics, this.chat, window, mouseX, mouseY, this.tickCount, this.getFont());
      }
   }

   public void extractScoreboardSidebar(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
   ...
   public void extractTabList(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
      Scoreboard scoreboard = this.minecraft.level.getScoreboard();
   ...
   public boolean canRenderCrosshairForSpectator(@Nullable HitResult hitResult) {
      if (hitResult == null) {
         return false;
   ...
   public void extractEffects(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
      ...
      if (!activeEffects.isEmpty() && (this.minecraft.gui.screen() == null || !this.minecraft.gui.screen().showsActiveEffects())) {
         ...
         for (MobEffectInstance instance : Ordering.natural().reverse().sortedCopy(activeEffects)) {
            ...
            if (renderer.isVisibleInGui(instance) && instance.showIcon()) {
               ...
               if (!renderer.extractGuiIcon(instance, this, graphics, x, y, 0.0F, alpha)) {
   ...
   public void extractItemHotbar(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
   ...
   public void extractSelectedItemName(GuiGraphicsExtractor graphics) {
      this.renderSelectedItemName(graphics, 0);
   }

   public final void updateContextualInfo(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
      Hud.ContextualInfo nextContextualInfo = this.nextContextualInfoState();
      if (nextContextualInfo != this.contextualInfoBar.getFirst()) {
         this.contextualInfoBar = Pair.of(nextContextualInfo, this.contextualInfoBars.get(nextContextualInfo).get());
      }
   ...
   public final void extractContextualInfoState(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
      ((ContextualBar)this.contextualInfoBar.getSecond()).extractRenderState(graphics, deltaTracker);
   }

   public void renderSelectedItemName(GuiGraphicsExtractor graphics, int yShift) {
      if (this.toolHighlightTimer > 0 && !this.lastToolHighlight.isEmpty()) {
         ...
         int strWidth = font.width(highlightTip);
   ...
   public void extractDemoOverlay(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
      if (this.minecraft.isDemo()) {
   ...
   public void displayScoreboardSidebar(GuiGraphicsExtractor graphics, Objective objective) {
   ...
   public @Nullable Player getCameraPlayer() {
      return this.minecraft.getCameraEntity() instanceof Player player ? player : null;
   }

   public @Nullable LivingEntity getPlayerVehicleWithHealth() {
      Player player = this.getCameraPlayer();
      if (player != null) {
   ...
   public int getVehicleMaxHearts(@Nullable LivingEntity vehicle) {
   ...
   public int getVisibleVehicleHeartRows(int hearts) {
      return (int)Math.ceil(hearts / 10.0);
   }

   public void extractPlayerHealth(GuiGraphicsExtractor graphics) {
      Player player = this.getCameraPlayer();
      if (player != null) {
   ...
   public static void extractArmor(GuiGraphicsExtractor graphics, Player player, int yLineBase, int numHealthRows, int healthRowHeight, int xLeft) {
   ...
   public void extractHeart(GuiGraphicsExtractor graphics, Hud.HeartType type, int xo, int yo, boolean isHardcore, boolean blinks, boolean half) {
      graphics.blitSprite(RenderPipelines.GUI_TEXTURED, type.getSprite(isHardcore, half, blinks), xo, yo, 9, 9);
   }

   public void extractAirBubbles(GuiGraphicsExtractor graphics, Player player, int vehicleHearts, int yLineAir, int xRight) {
   ...
   public int getAirBubbleYLine(int vehicleHearts, int yLineAir) {
      int rowOffset = this.getVisibleVehicleHeartRows(vehicleHearts) - 1;
      return yLineAir - rowOffset * 10;
   }

   public static int getCurrentAirSupplyBubble(int currentAirSupplyTicks, int maxAirSupplyTicks, int tickOffset) {
      return Mth.ceil((float)((currentAirSupplyTicks + tickOffset) * 10) / maxAirSupplyTicks);
   }

   public static int getEmptyBubbleDelayDuration(int currentAirSupplyTicks, boolean isUnderWater) {
      return currentAirSupplyTicks != 0 && isUnderWater ? 1 : 0;
   }

   public void playAirBubblePoppedSound(int bubble, Player player, int emptyAirBubbles) {
   ...
   public void extractFood(GuiGraphicsExtractor graphics, Player player, int yLineBase, int xRight) {
   ...
   public void extractVehicleHealth(GuiGraphicsExtractor graphics) {
      LivingEntity vehicleWithHearts = this.getPlayerVehicleWithHealth();
   ...
   public void extractTextureOverlay(GuiGraphicsExtractor graphics, Identifier texture, float alpha) {
   ...
   public void extractSpyglassOverlay(GuiGraphicsExtractor graphics, float scale) {
   ...
   public void updateVignetteBrightness(Entity camera) {
   ...
   public void extractVignette(GuiGraphicsExtractor graphics, @Nullable Entity camera) {
   ...
   public void extractPortalOverlay(GuiGraphicsExtractor graphics, float alpha) {
      if (alpha < 1.0F) {
         alpha *= alpha;
         alpha *= alpha;
   ...
   public void extractConfusionOverlay(GuiGraphicsExtractor graphics, float strength) {
   ...
   public void extractSlot(GuiGraphicsExtractor graphics, int x, int y, DeltaTracker deltaTracker, Player player, ItemStack itemStack, int seed) {
   ...
   public void tick() {
      ...
      if (this.minecraft.player != null) {
         ...
         if (selected.isEmpty()) {
            ...
            || !selected.getHighlightTip(selected.getHoverName()).equals(this.lastToolHighlight.getHighlightTip(this.lastToolHighlight.getHoverName()))) {
   ...
   public Hud.ContextualInfo nextContextualInfoState() {
