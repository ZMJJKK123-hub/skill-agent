...
public class AdvancementsScreen extends Screen implements ClientAdvancements.Listener {
   ...
   protected void init() {
      if (this.tabs.size() > AdvancementTabType.MAX_TABS) {
         ...
         StringWidget text = new StringWidget(Component.literal(String.format("%d / %d", maxPages + 1, maxPages + 1)), this.font);
         Consumer<Boolean> update = back -> {
         ...
         pageLayout.addChild(
            Button.builder(Component.literal("<"), b -> update.accept(true)).size(20, 20).build(),
            0,
            0,
            pageLayout.defaultCellSetting().alignHorizontallyLeft()
         );
         pageLayout.addChild(text, 0, 1, pageLayout.defaultCellSetting().alignHorizontallyCenter());
         pageLayout.addChild(
            Button.builder(Component.literal(">"), b -> update.accept(false)).size(20, 20).build(),
            0,
            2,
            pageLayout.defaultCellSetting().alignHorizontallyRight()
         );
   ...
   private void extractInside(GuiGraphicsExtractor graphics) {
      ...
      if (tab == null) {
         ...
         graphics.centeredText(this.font, NO_ADVANCEMENTS_LABEL, midX, this.topPos + 18 + 56 - 4, -1);
