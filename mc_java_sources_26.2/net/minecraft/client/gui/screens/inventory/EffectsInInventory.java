...
public class EffectsInInventory {
   ...
   public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
      ...
      if (!activeEffects.isEmpty() && availableWidth >= 32) {
         ScreenEvent.RenderInventoryMobEffects event = ForgeEventFactoryClient.onScreenEffectSize(this.screen, availableWidth, availableWidth < 120, xo);
         if (event == null) {
            return;
         }
         ...
         Iterable<MobEffectInstance> iterable = activeEffects.stream().filter(ForgeHooksClient::shouldRenderEffect).sorted().toList();
