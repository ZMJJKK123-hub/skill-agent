   ...
   public class KeyEntry extends KeyBindsList.Entry {
      ...
      private KeyEntry(final KeyMapping key, final Component name) {
         ...
         this.changeButton = Button.builder(name, button -> {
               ...
               key.setKeyModifierAndCode(null, InputConstants.UNKNOWN);
      ...
      public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float a) {
         ...
         graphics.text(KeyBindsList.this.minecraft.font, this.name, this.getContentX(), this.getContentYMiddle() - 4, -1);
      ...
      public void refreshEntry() {
         ...
         if (!this.key.isUnbound()) {
            for (KeyMapping otherKey : KeyBindsList.this.minecraft.options.keyMappings) {
               boolean vanillConflict = otherKey != this.key && this.key.same(otherKey) && (!otherKey.isDefault() || !this.key.isDefault());
               if (vanillConflict || otherKey.hasKeyModifierConflict(this.key)) {
                  if (this.hasCollision) {
                     tooltip.append(", ");
                  }
