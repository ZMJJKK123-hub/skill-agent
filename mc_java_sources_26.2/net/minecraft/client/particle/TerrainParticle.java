...
public class TerrainParticle extends SingleQuadParticle {
   ...
   public TerrainParticle(ClientLevel level, double x, double y, double z, double xa, double ya, double za, BlockState blockState, BlockPos pos) {
      ...
      if (tintSource != null && IClientBlockExtensions.of(blockState).areBreakingParticlesTinted(blockState, level, pos)) {
   ...
   public Particle updateSprite(BlockState state, BlockPos pos) {
      if (pos != null) {
         this.setSprite(
            Minecraft.getInstance().getModelManager().getBlockStateModelSet().get(state).particleMaterial(this.level.getModelDataManager().getAt(pos)).sprite()
         );
      }
