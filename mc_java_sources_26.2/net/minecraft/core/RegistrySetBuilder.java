...
public class RegistrySetBuilder implements IForgeRegistrySetBuilder {
   private final Object2IntMap<ResourceKey<? extends Registry<?>>> indexes = new Object2IntLinkedOpenHashMap();
   ...
   public <T> RegistrySetBuilder add(ResourceKey<? extends Registry<T>> key, Lifecycle lifecycle, RegistrySetBuilder.RegistryBootstrap<T> bootstrap) {
      this.add(new RegistrySetBuilder.RegistryStub<>(key, lifecycle, bootstrap));
      return this;
   }

   private <T> RegistrySetBuilder.RegistryStub<T> add(RegistrySetBuilder.RegistryStub<T> entry) {
      int idx = this.indexes.getOrDefault(entry.key(), -1);
      if (idx != -1) {
         RegistrySetBuilder.RegistryBootstrap<T> _oldB = (RegistrySetBuilder.RegistryBootstrap<T>)this.entries.get(idx).bootstrap();
         RegistrySetBuilder.RegistryBootstrap<T> _newB = entry.bootstrap();
         RegistrySetBuilder.RegistryStub<T> _new = new RegistrySetBuilder.RegistryStub<>(entry.key(), entry.lifecycle(), ctx -> {
            _oldB.run(ctx);
            _newB.run(ctx);
         });
   ...
   public RegistrySetBuilder copy() {
      RegistrySetBuilder ret = new RegistrySetBuilder();
   ...
   private <T> HolderLookup.RegistryLookup<T> createLazyFullPatchedRegistries(
      ...
      HolderLookup.RegistryLookup<T> patchContents = (HolderLookup.RegistryLookup<T>)patchProvider.lookup(registryKey).orElse(null);
      ...
      HolderLookup.RegistryLookup<T> fallbackContents = (HolderLookup.RegistryLookup<T>)fallbackProvider.lookup(registryKey).orElse(null);
