...
public class FuelValues {
   ...
   public boolean isFuel(ItemStack itemStack) {
      return this.isFuel(itemStack, null);
   }

   public boolean isFuel(ItemStack stack, @Nullable RecipeType<?> type) {
      return this.burnDuration(stack, type) > 0;
   }
   ...
   public int burnDuration(ItemStack itemStack) {
      return this.burnDuration(itemStack, null);
   }

   public int burnDuration(ItemStack itemStack, @Nullable RecipeType<?> type) {
      if (itemStack.isEmpty()) {
         return 0;
      }

      int ret = itemStack.getBurnTime(type);
      if (ret == -1) {
         ret = this.values.getInt(itemStack.getItem());
      }

      return ForgeEventFactory.getItemBurnTime(itemStack, ret, type);
   }
