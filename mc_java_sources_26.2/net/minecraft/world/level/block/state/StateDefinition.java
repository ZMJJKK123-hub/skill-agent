...
public class StateDefinition<O, S extends StateHolder<O, S>> {
   ...
   private static <O, S extends StateHolder<O, S>> ImmutableList<S> createMultiPropertyStates(
      ...
      statesByValues.forEach((valuesx, state) -> state.initializeNeighbors(stateCollection.fillNeighborsForState(propertyKeys, (List<Comparable<?>>)valuesx)));
      return states.build();
   }
   ...
   private static <S extends StateHolder<?, S>, T extends Comparable<T>> MapCodec<S> appendPropertyCodec(
      ...
      return Codec.mapPair(codec, property.valueCodec().fieldOf(name).orElseGet(var0x -> {}, () -> property.value(defaultSupplier.get())))
         .xmap(
   ...
   public interface Factory<O, S> {
      S create(O var1, Property<?>[] var2, Comparable<?>[] var3);
   }
