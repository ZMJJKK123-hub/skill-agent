   ...
   public void doServerTick(ServerLevel level) {
      ...
      if (attackTarget != null) {
         if (this.scanningTime > 25) {
            ...
            float angle = (float)(Math.acos(dot) * 180.0 / (float) Math.PI) + 0.5F;
            if (angle < 0.0F || angle > 10.0F) {
               ...
               double yRotDelta = Mth.clamp(
                  Mth.wrapDegrees(180.0 - Mth.atan2(xAttackDist, zAttackDist) * 180.0 / (float) Math.PI - this.dragon.getYRot()), -100.0, 100.0
               );
