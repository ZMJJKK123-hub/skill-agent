...
public class HoeItem extends Item {
   ...
   protected static final Map<Block, Pair<Predicate<UseOnContext>, Consumer<UseOnContext>>> TILLABLES = Maps.newHashMap(
      ImmutableMap.of(
         ...
         Blocks.ROOTED_DIRT,
         Pair.of((Predicate<UseOnContext>)context -> true, changeIntoStateAndDropItem(Blocks.DIRT.defaultBlockState(), Items.HANGING_ROOTS))
      )
   );
   ...
   public InteractionResult useOn(UseOnContext context) {
      ...
      Pair<Predicate<UseOnContext>, Consumer<UseOnContext>> logicPair = toolModifiedState == null
         ? null
         : Pair.of((Predicate<UseOnContext>)ctx -> true, changeIntoState(toolModifiedState));
      if (logicPair == null) {
         return InteractionResult.PASS;
      }

      Predicate<UseOnContext> predicate = (Predicate<UseOnContext>)logicPair.getFirst();
      Consumer<UseOnContext> action = (Consumer<UseOnContext>)logicPair.getSecond();
