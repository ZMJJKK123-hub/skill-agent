...
public class BowItem extends ProjectileWeaponItem {
   ...
   public boolean releaseUsing(ItemStack itemStack, Level level, LivingEntity entity, int remainingTime) {
      if (entity instanceof Player player) {
         ...
         timeHeld = ForgeEventFactory.onArrowLoose(itemStack, level, player, timeHeld, true);
         if (timeHeld < 0) {
            return false;
         }
   ...
   public InteractionResult use(Level level, Player player, InteractionHand hand) {
      ...
      InteractionResult ret = ForgeEventFactory.onArrowNock(itemStack, level, player, hand, foundProjectile);
      if (ret != null) {
         return ret;
      }
