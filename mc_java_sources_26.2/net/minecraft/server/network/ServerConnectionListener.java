...
public class ServerConnectionListener {
   ...
   public void startTcpServerListener(@Nullable InetAddress address, int port) throws IOException {
      InetAddress fixedAddress = address != null ? address : new InetSocketAddress(port).getAddress();
      DualStackUtils.checkIPv6(fixedAddress);
         ...
         this.channels
            .add(
               ((ServerBootstrap)((ServerBootstrap)new ServerBootstrap().channel(eventLoopGroupHolder.serverChannelCls()))
                     .childHandler(
                        new ChannelInitializer<Channel>() {
                           protected void initChannel(Channel channel) {
                              ...
                              ChannelPipeline pipeline = channel.pipeline().addLast("timeout", new ReadTimeoutHandler(ServerConnectionListener.READ_TIMEOUT));
                     ...
                     .localAddress(fixedAddress, port))
                  .bind()
                  .syncUninterruptibly()
            );
      }
