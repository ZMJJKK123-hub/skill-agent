...
public class ExperienceCommand {
   ...
   private static int addExperience(CommandSourceStack source, Collection<? extends ServerPlayer> players, int amount, ExperienceCommand.Type type) {
      ...
      if (players.size() == 1) {
         source.sendSuccess(
            () -> Component.translatable("commands.experience.add." + type.name + ".success.single", amount, players.iterator().next().getDisplayName()), true
         );
      } else {
   ...
   private static int setExperience(CommandSourceStack source, Collection<? extends ServerPlayer> players, int amount, ExperienceCommand.Type type) throws CommandSyntaxException {
      ...
      if (players.size() == 1) {
         source.sendSuccess(
            () -> Component.translatable("commands.experience.set." + type.name + ".success.single", amount, players.iterator().next().getDisplayName()), true
         );
      } else {
