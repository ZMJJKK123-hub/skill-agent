...
public abstract class PackDetector<T> {
   ...
   public @Nullable T detectPackResources(Path content, List<ForbiddenSymlinkInfo> issues) throws IOException {
      return this.detectPackResources(content, issues, true);
   }

   public @Nullable T detectPackResources(Path content, List<ForbiddenSymlinkInfo> issues, boolean requireMeta) throws IOException {
      ...
      if (attributes.isDirectory()) {
         ...
         String name = targetContext.getFileName().toString();
         return !attributes.isRegularFile() || !name.endsWith(".zip") && !name.endsWith(".jar") ? null : this.createZipPack(targetContext);
      }
   }
