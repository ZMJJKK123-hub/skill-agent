...
public class Main {
   ...
   public static void main(String[] args) {
      ...
      parser.accepts("allowUpdates").withRequiredArg().ofType(Boolean.class).defaultsTo(Boolean.TRUE, new Boolean[0]);
      parser.accepts("gameDir").withRequiredArg().ofType(File.class).defaultsTo(new File("."), new File[0]);
      OptionSpec<Void> uniqueWorld = null;
      boolean gametestEnabled = Boolean.getBoolean("forge.gameTestServer");
      if (gametestEnabled) {
         OptionSpec<BlockPos> spawnPosOpt = parser.accepts("spawnPos")
            .withRequiredArg()
            .withValuesConvertedBy(new BlockPosValueConverter())
            .defaultsTo(new BlockPos(0, 60, 0), new BlockPos[0]);
         uniqueWorld = parser.accepts("uniqueWorld");
      } else {
         OptionSpec<BlockPos> spawnPosOpt = null;
      }
      ...
      try {
         ...
         if (uniqueWorld != null && options.has(uniqueWorld)) {
            levelName = "gametest_world\\" + DateTimeFormatter.ofPattern("yyyy-MM-dd_HH.mm.ss.SSS").format(LocalDateTime.now());
         }
         ...
         if (levelDataTag != null) {
            ForgeHooks.readAdditionalLevelSaveData(access, access.getLevelDirectory());
         }
   ...
   private static WorldLoader.DataLoadOutput<LevelDataAndDimensions.WorldDataAndGenSettings> createNewWorldData(
      ...
      dimensions = WorldDimensions.CODEC
         .encoder()
         .encodeStart(dynamicops, dimensions)
         .flatMap(writtenPayloadWithModdedDimensions -> WorldDimensions.CODEC.decoder().parse(dynamicops, writtenPayloadWithModdedDimensions))
