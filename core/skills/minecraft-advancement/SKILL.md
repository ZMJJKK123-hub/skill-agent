...
description: "Minecraft Advancement 进度定义格式：JSON 数据包结构（data/<namespace>/advancement/<name>.json）、Root Format 根格式（criteria/display/parent/requirements/rewards/sends_telemetry_event）、Display 显示信息（announce_to_chat/background/description/frame/hidden/icon/show_toast/title）、Tabs 标签页（root advancement 自动创建）、Layout 布局（自动排列、箭头连接）、Criteria 标准（trigger 触发器 ID、conditions 条件、player 实体谓词）、Triggers 触发器列表（allay_drop_item_on_block/any_block_use/avoid_vibration/bee_nest_destroyed/bred_animals/brewed_potion/changed_dimension/channeled_lightning/construct_beacon/consume_item/crafter_recipe_crafted/cured_zombie_villager/default_block_use/effects_changed/enchanted_item/enter_block/entity_hurt_player/entity_killed_player/fall_after_explosion/fall_from_height/filled_bucket/fishing_rod_hooked/hero_of_the_village/impossible/inventory_changed/item_durability_changed/item_used_on_block/kill_mob_near_sculk_catalyst/killed_by_arrow/levitation/lightning_strike/location/nether_travel/placed_block/player_generates_container_loot/player_hurt_entity/player_interacted_with_entity/player_killed_entity/player_sheared_equipment/recipe_crafted/recipe_unlocked/ride_entity_in_lava/shot_crossbow/slept_in_bed/slide_down_block/started_riding/summoned_entity/spear_mobs/tame_animal/target_hit/thrown_item_picked_up_by_entity/thrown_item_picked_up_by_player/tick/used_ender_eye/used_totem/using_item/villager_trade/voluntary_exile）、Removed Triggers 已移除触发器（arbitrary_player_tick/item_delivered_to_player/player_damaged/safely_harvest_honey/killed_by_crossbow）、Advancement Generator 可视化编辑器。"
