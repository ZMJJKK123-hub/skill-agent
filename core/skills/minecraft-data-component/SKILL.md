...
description: "Minecraft Data Component 数据组件格式：Behavior 行为（组件编码、持久化/同步属性、隐式组件 NBT 绑定）、Item Stacks 物品堆栈（组件补丁 patch 数据、! 前缀移除默认组件、id/components/count 字段）、Block Entities 方块实体（部分组件使用、隐式组件 NBT 存储、components 字段）、Entities 实体（隐式组件存储、应用优先级 bucket_entity_data→entity_data→其他）、Data Component Types 数据组件类型列表（attack_animation/attack_range/attribute_modifiers/banner_patterns/base_color/bees/block_entity_data/block_state/block_transformer/blocks_attacks/break_sound/brewing_fuel/bucket_entity_data/bundle_contents/can_break/can_place_on/charged_projectiles/compostable/consumable/container/container_loot/cooking_fuel/custom_data/custom_model_data/custom_name/damage/damage_resistant/damage_type/death_protection/debug_stick_state/dye/dyed_color/enchantable/enchantment_glint_override/enchantments/stored_enchantments/entity_data/equippable/firework_explosion/fireworks/food/glider/instrument/intangible_projectile/interact_animation/item_model/item_name/jukebox_playable/kinetic_weapon/lock/lodestone_tracker/lore/map_color/map_decorations/map_id/max_damage/max_stack_size/minimum_attack_charge/mob_visibility/note_block_sound/ominous_bottle_amplifier/piercing_weapon/pot_decorations/potion_contents/potion_duration_scale/profile/provides_banner_patterns/provides_pottery_pattern/provides_trim_material/rarity/recipes/repair_cost/repairable/sign_text_front/sign_text_back/sulfur_cube_content/suspicious_stew_effects/swing_animation/tool/tooltip_display/tooltip_style/trim/unbreakable/use_cooldown/use_effects/use_remainder/villager_food/waxed/weapon/writable_book_content/written_book_content）、Entity Variant Components 实体变体组件、Temporary Components 临时组件（additional_trade_cost/creative_slot_lock/map_post_processing）、Removed Components 已移除组件。"
