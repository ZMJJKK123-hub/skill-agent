...
description: "Minecraft Environment Attributes 环境属性：Sources and Modifiers 来源与修饰符（4种来源：dimension 维度/biome 生物群系/timeline 时间线/weather 天气，按顺序应用；修饰符 override/and/nand/or/nor/xor/xnor/add/subtract/multiply/minimum/maximum/alpha_blend/blend_to_gray）、Interpolation 插值（可插值属性平滑过渡，相机位置属性 半径10方块生物群系体积贡献，时间线属性轨迹 插值）、Technical Details 技术细节（3个独立系统：Server world 环境属性、Client world 环境属性+闪电闪烁 sky_color 插值 #CCF0FF、未完成区块环境属性 26.3后只有维度+生物群系来源）、Definition Format 定义格式（Attribute map 在维度类型和生物群系定义的 attributes 字段、Attribute track 在时间线定义文件）、Common Types 常见类型（RGB color #rrggbb/int/float[3]、ARGB color #aarrggbb/int/float[4]）、Attribute List 属性列表（Visual 视觉：fog_color/fog_start_distance/fog_end_distance/sky_fog_end_distance/cloud_fog_end_distance/water_fog_color/water_fog_start_distance/water_fog_end_distance/sky_color/sunrise_sunset_color/cloud_color/cloud_height/sun_angle/moon_angle/star_angle/moon_phase/star_brightness/block_light_tint/sky_light_color/sky_light_factor/night_vision_color/ambient_light_color/default_dripstone_particle/ambient_particles；Audio 音频：background_music/music_volume/ambient_sounds/firefly_bush_sounds；Gameplay 游戏玩法：sky_light_level/can_start_raid/water_evaporates/bed_rule/respawn_anchor_works/nether_portal_spawns_piglin/fast_lava/increased_fire_burnout/eyeblossom_open/turtle_egg_hatch_chance/piglins_zombify/snow_golem_melts/creaking_active/surface_slime_spawn_chance/cat_waking_up_gift_chance/bees_stay_in_hive/monsters_burn/can_pillager_patrol_spawn/villager_activity/baby_villager_activity）、Modifiers 修饰符、Weather Attributes 天气属性、Other Uses 其他用途（loot predicate environment_attribute_check、number provider environment_attribute）。"
