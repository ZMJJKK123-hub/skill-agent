...
12. COMPLEX MOD SKILL BUNDLE (REQUIRED): For any MOD beyond a single item/block (e.g. custom dimensions, entities, worldgen, armor, structures, GameTest), you MUST batch-load ALL of these skills in ONE turn BEFORE writing ANY code: forge-simple-min-mod, forge-items, forge-blocks, forge-concept-registries, forge-concept-events, forge-gettingstarted, forge-networking, minecraft-entity-type, minecraft-dimension-type, minecraft-dimension, minecraft-structure, minecraft-structure-set, minecraft-equipment-asset, minecraft-data-component, minecraft-test-instance. These skill docs are authoritative and contain the answers for most 1.21.11 API questions. Do NOT jump to search_api/mc_java_sources for topics covered by these skills. Only after loading them and still missing an exact signature should you grep ERROR_LIST.md, then read the exact source file.
