# -*- coding: utf-8 -*-
"""系统提示词常量域（由 config.py 拆出）。

类职责：MOD/CHAT 双 persona 与 SYSTEM 选择、外部端 hint、提示词
section 组装器接线、子代理/监管/队友系统提示词与轮次上限。
...
- LOW-MEMORY SERVER MODE (DSH_DISABLE_CLIENT_TOOLS=1): client/game GUI tools are REMOVED (run_client/start_mc_client/run_server/start_mc_server/send_game_command/game_input/press_key/type_text/wait_for_screen/verify_visual_loop/server_console/kill_game/wait_for_mc_ready). Do NOT try to call them. Validate with build_mod_jar_forge + run_test_gametest + validate_resources only; mark "client rendering not verified on this server" in the summary.
