...
public class RealmsWorldSlotButton extends Button {
   ...
   public RealmsWorldSlotButton(
      RealmsConfigureWorldScreen configureWorldScreen, int x, int y, int width, int height, int slotIndex, RealmsServer serverData, Button.OnPress onPress
   ) {
   ...
   private static RealmsWorldSlotButton.Action getAction(boolean activeSlot, boolean empty, boolean expired) {
      return !activeSlot && (!empty || !expired) ? RealmsWorldSlotButton.Action.SWITCH_SLOT : RealmsWorldSlotButton.Action.NOTHING;
   }
