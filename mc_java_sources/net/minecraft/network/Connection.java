...
public class Connection extends SimpleChannelInboundHandler<Packet<?>> {
   ...
   private final PacketLogger packetLogger = new PacketLogger(this);
            ...
            if (cause instanceof TimeoutException) {
               ...
               if (isFirstFault) {
                  ...
                  if (this.getSending() == PacketFlow.CLIENTBOUND) {
                     Packet<?> packet = this.sendLoginDisconnect ? new ClientboundLoginDisconnectPacket(reason) : new ClientboundDisconnectPacket(reason);
   ...
   public void setupOutboundProtocol(ProtocolInfo<?> protocol) {
      ...
      UnconfiguredPipelineHandler.OutboundConfigurationTask configMessage = UnconfiguredPipelineHandler.setupOutboundProtocol(protocol)
         .andThen(var2x -> this.outboundProtocol = protocol);
      BundlerInfo bundlerInfo = protocol.bundlerInfo();
      if (bundlerInfo != null) {
      ...
      this.runOnceConnected(
         connection -> {
            ...
            connection.sendPacket(
               new ClientIntentionPacket(SharedConstants.getCurrentVersion().protocolVersion(), NetworkContext.enhanceHostName(hostName), port, intent),
               null,
               true
            );
   ...
   private void doSendPacket(Packet<?> packet, @Nullable ChannelFutureListener listener, boolean flush) {
      if (listener != null) {
         ...
         future.addListener(f -> this.packetLogger.send(packet));
      } else if (flush) {
         this.channel.writeAndFlush(packet, this.channel.voidPromise());
         this.packetLogger.send(packet);
      } else {
   ...
   public String getLoggableAddress(boolean logIPs) {
      if (this.address == null) {
         ...
         return logIPs ? DualStackUtils.getAddressString(this.address) : "IP hidden";
      }
   }
   ...
   public static ChannelFuture connect(InetSocketAddress address, EventLoopGroupHolder eventLoopGroupHolder, final Connection connection) {
      ...
      return ((Bootstrap)((Bootstrap)((Bootstrap)new Bootstrap().group(eventLoopGroupHolder.eventLoopGroup(true))).handler(new ChannelInitializer<Channel>() {
   ...
   public static void configureSerialization(ChannelPipeline pipeline, PacketFlow inboundDirection, boolean local, @Nullable BandwidthDebugMonitor monitor) {
      ...
      pipeline.addLast("splitter", createFrameDecoder(monitor, local))
         ...
         .addLast(
            ...
            (ChannelHandler)(configureInbound ? new PacketDecoder<ServerHandshakePacketListener>(INITIAL_PROTOCOL) : new UnconfiguredPipelineHandler.Inbound())
         )
         .addLast("prepender", createFrameEncoder(local))
         .addLast(
            outboundHandlerName(configureOutbound),
            (ChannelHandler)(configureOutbound
               ? new PacketEncoder<ServerHandshakePacketListener>(INITIAL_PROTOCOL)
   ...
   public static Connection connectToLocalServer(SocketAddress address) {
      ...
      connection.activationHandler = NetworkRegistry::onConnectionStart;
      ((Bootstrap)((Bootstrap)((Bootstrap)new Bootstrap().group(EventLoopGroupHolder.local().eventLoopGroup(true))).handler(new ChannelInitializer<Channel>() {
         protected void initChannel(Channel channel) {
