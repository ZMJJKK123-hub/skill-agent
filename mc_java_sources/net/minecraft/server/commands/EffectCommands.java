...
public class EffectCommands {
                        ...
                        ((RequiredArgumentBuilder)Commands.argument("targets", EntityArgument.entities())
                           ...
                           .then(
                              Commands.argument("effect", ResourceArgument.resource(context, Registries.MOB_EFFECT))
                                 .executes(
                                    c -> clearEffect(
                                       (CommandSourceStack)c.getSource(), EntityArgument.getEntities(c, "targets"), ResourceArgument.getMobEffect(c, "effect")
                                    )
                                 )
                           )
                     )
   ...
   private static int clearEffects(CommandSourceStack source, Collection<? extends Entity> entities) throws CommandSyntaxException {
      ...
      if (entities.size() == 1) {
         source.sendSuccess(() -> Component.translatable("commands.effect.clear.everything.success.single", entities.iterator().next().getDisplayName()), true);
      } else {
   ...
   private static int clearEffect(CommandSourceStack source, Collection<? extends Entity> entities, Holder<MobEffect> effectHolder) throws CommandSyntaxException {
      ...
      if (entities.size() == 1) {
         source.sendSuccess(
            () -> Component.translatable("commands.effect.clear.specific.success.single", effect.getDisplayName(), entities.iterator().next().getDisplayName()),
            true
         );
      } else {
