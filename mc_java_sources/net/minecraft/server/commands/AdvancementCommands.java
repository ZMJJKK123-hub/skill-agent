   ...
   private static int perform(
      ...
      if (advancementCount == 0) {
         if (advancements.size() == 1) {
            ...
            if (players.size() == 1) {
               throw ERROR_NO_ACTION_PERFORMED.create(
                  Component.translatable(
                     action.getKey() + ".one.to.one.failure", advancementName, ((ServerPlayer)Iterables.getOnlyElement(players)).getDisplayName()
                  )
               );
            } else {
            ...
            throw ERROR_NO_ACTION_PERFORMED.create(
               Component.translatable(
                  action.getKey() + ".many.to.one.failure", advancements.size(), ((ServerPlayer)Iterables.getOnlyElement(players)).getDisplayName()
               )
            );
         } else {
         ...
         if (advancements.size() == 1) {
            ...
            if (players.size() == 1) {
               source.sendSuccess(
                  () -> Component.translatable(
                     action.getKey() + ".one.to.one.success", advancementName, ((ServerPlayer)Iterables.getOnlyElement(players)).getDisplayName()
                  ),
                  true
               );
            } else {
            ...
            if (players.size() == 1) {
               source.sendSuccess(
                  () -> Component.translatable(
                     action.getKey() + ".many.to.one.success", finalAdvancementCount, ((ServerPlayer)Iterables.getOnlyElement(players)).getDisplayName()
                  ),
                  true
               );
            } else {
         ...
         if (players.size() == 1) {
            throw ERROR_NO_ACTION_PERFORMED.create(
               Component.translatable(
                  ...
                  criterion,
                  Advancement.name(holder),
                  ((ServerPlayer)Iterables.getOnlyElement(players)).getDisplayName()
               )
            );
         } else {
            throw ERROR_NO_ACTION_PERFORMED.create(
