...
public class SpreadPlayersCommand {
   ...
   private static int spreadPlayers(
      CommandSourceStack source, Vec2 center, float spreadDistance, float maxRange, int maxHeight, boolean respectTeams, Collection<? extends Entity> entities
   ...
   private static double setPlayerPositions(
      ...
      for (Entity entity : entities) {
         ...
         EntityTeleportEvent.SpreadPlayersCommand event = ForgeEventFactory.onEntityTeleportSpreadPlayersCommand(
            entity, Mth.floor(position.x) + 0.5, position.getSpawnY(level, maxHeight), Mth.floor(position.z) + 0.5
         );
         if (event != null) {
            entity.teleportTo(level, event.getTargetX(), event.getTargetY(), event.getTargetZ(), Set.of(), entity.getYRot(), entity.getXRot(), true);
         }
