      ...
      return ExtraCodecs.retrieveContext(
         ops -> ops instanceof RegistryOps<?> registryOps
            ? registryOps.lookupProvider
               .lookup(resourceKey)
               .map(
                  registryInfo -> registryInfo.owner() instanceof HolderLookup.RegistryLookup<E> registryLookup
                     ? DataResult.success(registryLookup, registryInfo.elementsLifecycle())
                     : DataResult.error(() -> "Found holder getter but was not a registry lookup for " + resourceKey)
               )
