...
public class BuiltInRegistries {
   ...
   private static <T, R extends WritableRegistry<T>> R internalRegister(
      ...
      R maybeWrapped = GameData.getWrapper(name, registry);
      registry = maybeWrapped;
      LOADERS.put(key, () -> loader.run(maybeWrapped));
   ...
   private static <T extends Registry<?>> void validate(Registry<T> registry) {
      registry.forEach(r -> {
         ...
         if (r instanceof DefaultedRegistry) {
            ...
            Objects.requireNonNull(r.getValue(key), "Missing default of DefaultedMappedRegistry: " + r.key() + " - " + key);
         }
      });
   }
