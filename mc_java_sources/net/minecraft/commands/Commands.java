...
public class Commands {
   ...
   public Commands(Commands.CommandSelection commandSelection, CommandBuildContext context) {
      ...
      if (SharedConstants.DEBUG_DEV_COMMANDS || ForgeGameTestHooks.isGametestEnabled()) {
      ...
      ForgeEventFactory.onCommandRegister(this.dispatcher, commandSelection, context);
   ...
   public void performCommand(ParseResults<CommandSourceStack> command, String commandString) {
      ...
      try {
         ...
         if (!CommandEvent.BUS.post(event)) {
            ContextChain<CommandSourceStack> commandChain = finishParsing(event.getParseResults(), commandString, sender);
   ...
   public void sendCommands(ServerPlayer player) {
      ...
      CommandHelper.mergeCommandNode(this.dispatcher.getRoot(), root, playerCommands, player.createCommandSourceStack(), ctx -> 0, suggest -> suggest);
      root = CommandHelper.filterCommandList(player.connection.getConnection(), root);
