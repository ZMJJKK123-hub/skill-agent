...
public class ChatListener {
   ...
   public void handleDisguisedChatMessage(Component message, ChatType.Bound boundChatType) {
      ...
      this.handleMessage(null, () -> {
         ...
         if (receiver != null && receiver.chatAbilities().canReceivePlayerMessages()) {
            ...
            decoratedMessage = ForgeHooksClient.onClientChat(boundChatType, decoratedMessage, Util.NIL_UUID);
      ...
      if (!this.minecraft.isBlocked(message.sender()) && !message.isFullyFiltered()) {
         ...
         if (receiver != null && receiver.chatAbilities().canReceivePlayerMessages()) {
            ...
            if (filterMask.isEmpty()) {
               Component forgeComponent = ForgeHooksClient.onClientPlayerChat(boundChatType, decoratedMessage, message, message.sender());
               ...
               if (filteredContent != null) {
                  Component forgeComponent = ForgeHooksClient.onClientPlayerChat(
                     boundChatType, boundChatType.decorate(filteredContent), message, message.sender()
                  );
                  if (forgeComponent == null) {
                     return false;
                  }
   ...
   public void handleSystemMessage(Component message, boolean remote) {
      ...
      if ((!this.minecraft.options.hideMatchedNames().get() || !this.minecraft.isBlocked(guessedUUID))
         && (guessedUUID == Util.NIL_UUID || !this.minecraft.isFriendOnlyRestricted(guessedUUID))) {
         LocalPlayer receiver = this.minecraft.player;
         if (receiver != null && receiver.chatAbilities().canReceiveSystemMessages()) {
            message = ForgeHooksClient.onClientSystemMessage(message, remote);
            if (message == null) {
               return;
            }
