...
public class HumanoidModel<T extends HumanoidRenderState> extends EntityModel<T> implements ArmedModel<T>, HeadedModel {
   ...
   private void poseRightArm(T state) {
      switch (state.rightArmPose) {
         ...
         default:
            state.rightArmPose.applyTransform(this, state, HumanoidArm.RIGHT);
      }
   }
   ...
   private void poseLeftArm(T state) {
      switch (state.leftArmPose) {
         ...
         default:
            state.leftArmPose.applyTransform(this, state, HumanoidArm.LEFT);
      }
   }
   ...
   public enum ArmPose implements IExtensibleEnum {
      ...
      ArmPose(boolean twoHanded, boolean affectsOffhandPose, @Nonnull IArmPoseTransformer forgeArmPose) {
         this.twoHanded = twoHanded;
         this.affectsOffhandPose = affectsOffhandPose;
      ...
      public static HumanoidModel.ArmPose create(String name, boolean twoHanded, boolean affectsOffhandPose, @Nonnull IArmPoseTransformer forgeArmPose) {
         throw new IllegalStateException("Enum not extended");
      }

      public <T extends HumanoidRenderState> void applyTransform(HumanoidModel<T> model, T state, HumanoidArm arm) {
         if (this.forgeArmPose != null) {
            this.forgeArmPose.applyTransform(model, state, arm);
         }
      }
