...
public class AvatarRenderer<AvatarlikeEntity extends Avatar & ClientAvatarEntity>
   ...
   private static HumanoidModel.ArmPose getArmPose(Avatar avatar, ItemStack itemInHand, InteractionHand hand) {
      ...
      if (!avatar.swinging && itemInHand.getItem() instanceof CrossbowItem && CrossbowItem.isCharged(itemInHand)) {
      ...
      HumanoidModel.ArmPose pose = IClientItemExtensions.of(itemInHand).getArmPose(avatar, hand, itemInHand);
   ...
   public void renderRightHand(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, Identifier skinTexture, boolean hasSleeve) {
      if (!ForgeEventFactoryClient.onRenderArm(poseStack, submitNodeCollector, lightCoords, HumanoidArm.RIGHT)) {
   ...
   public void renderLeftHand(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, Identifier skinTexture, boolean hasSleeve) {
      if (!ForgeEventFactoryClient.onRenderArm(poseStack, submitNodeCollector, lightCoords, HumanoidArm.LEFT)) {
   ...
   public void submit(AvatarRenderState state, PoseStack pose, SubmitNodeCollector source, CameraRenderState camera) {
      if (!ForgeEventFactoryClient.onRenderAvatarPre(state, this, pose, source, camera)) {
         super.submit(state, pose, source, camera);
         ForgeEventFactoryClient.onRenderAvatarPost(state, this, pose, source, camera);
      }
   }
}
