...
public class LevelRenderer implements AutoCloseable {
   ...
   public void render(
      ...
      FramePassManager.insertForgePasses(frame, this.targets, this.levelRenderState, deltaTracker);
   ...
   private void submitFeatures(LevelRenderState levelRenderState, SubmitNodeCollector submitNodeCollector, boolean renderOutline) {
      ...
      boolean hasCustomOutline = this.levelRenderState.blockOutlineRenderState != null
         && this.levelRenderState.blockOutlineRenderState.customRenderer() != null;
      if (renderOutline || hasCustomOutline) {
      ...
      if (!levelRenderState.shouldShowEntityOutlines) {
         ObjectIterator var6 = this.submitNodeStorage.getSubmitsPerOrder().values().iterator();

         while (var6.hasNext()) {
            SubmitNodeCollection collection = (SubmitNodeCollection)var6.next();
   ...
   private void submitBlockOutline(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, LevelRenderState levelRenderState) {
      ...
      if (state != null) {
         if (state.customRenderer() != null) {
            state.customRenderer().render(submitNodeCollector, poseStack, levelRenderState);
            return;
         }
