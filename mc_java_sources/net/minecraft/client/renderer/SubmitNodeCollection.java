...
public class SubmitNodeCollection implements OrderedSubmitNodeCollector {
   ...
   public <S> void submitModel(
      ...
      if (outlineColor != 0) {
         ...
         if (outlineRenderType != null) {
            this.outline
               .submit(
                  new ModelFeatureRenderer.Submit<>(outlineRenderType, pose, model, state, 15728880, OverlayTexture.NO_OVERLAY, outlineColor, sprite, null)
               );
         }
      }
   ...
   public void submitBlockModel(
      PoseStack poseStack, RenderType renderType, List<BlockStateModelPart> modelParts, int[] tintLayers, int lightCoords, int overlayCoords, int outlineColor
   ) {
      PoseStack.Pose pose = poseStack.last().copy();
      if (!renderType.isOutline()) {
