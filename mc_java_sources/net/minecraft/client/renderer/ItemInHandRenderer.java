...
public class ItemInHandRenderer {
   ...
   private void renderOneHandedMap(
      PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, float inverseArmHeight, HumanoidArm arm, float attackValue, ItemStack map
   ) {
      float invert = arm == HumanoidArm.RIGHT ? 1.0F : -1.0F;
   ...
   public void submitHandsWithItems(float frameInterp, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, LocalPlayer player, int lightCoords) {
      ...
      if (handRenderSelection.renderMainHand) {
         ...
         if (!ForgeHooksClient.renderSpecificFirstPersonHand(
      ...
      if (handRenderSelection.renderOffHand) {
         ...
         if (!ForgeHooksClient.renderSpecificFirstPersonHand(
            InteractionHand.OFF_HAND, poseStack, submitNodeCollector, lightCoords, frameInterp, xRot, offHandAttack, offhandInverseArmHeight, this.offHandItem
         )) {
            this.submitArmWithItem(
               player,
               frameInterp,
   ...
   private void submitArmWithItem(
      ...
      if (!player.isScoping()) {
         ...
         if (itemStack.isEmpty()) {
            ...
            boolean isRightArm = arm == HumanoidArm.RIGHT;
            int invert = isRightArm ? 1 : -1;
            if (!IClientItemExtensions.of(itemStack)
               .applyForgeHandTransform(poseStack, this.minecraft.player, arm, itemStack, frameInterp, inverseArmHeight, attack)) {
   ...
   public void tick() {
      ...
      if (player.isHandsBusy()) {
         ...
         boolean requipM = ForgeHooksClient.shouldCauseReequipAnimation(this.mainHandItem, nextMainHand, player.getInventory().getSelectedSlot());
         boolean requipO = ForgeHooksClient.shouldCauseReequipAnimation(this.offHandItem, nextOffHand, -1);
         if (!requipM && this.mainHandItem != nextMainHand) {
            this.mainHandItem = nextMainHand;
         }

         if (!requipO && this.offHandItem != nextOffHand) {
            this.offHandItem = nextOffHand;
         }

         float mainHandTargetHeight = requipM ? 0.0F : attackAnim * attackAnim * attackAnim;
         float offHandTargetHeight = requipO ? 0.0F : 1.0F;
   ...
   enum HandRenderSelection {
      ...
      HandRenderSelection(final boolean renderMainHand, final boolean renderOffHand) {
