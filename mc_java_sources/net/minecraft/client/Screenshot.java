   ...
   public static void grab(File workDir, @Nullable String forceName, RenderTarget target, int downscaleFactor, Consumer<Component> callback) {
      takeScreenshot(
         ...
         image -> {
            ...
            ScreenshotEvent event = new ScreenshotEvent(image, file);
            if (ScreenshotEvent.BUS.post(event)) {
               callback.accept(event.getCancelMessage());
            } else {
               File outputFile = event.getScreenshotFile();
               Util.ioPool()
                  .execute(
                     () -> {
                        try (image) {
                           ...
                           Component component = Component.literal(outputFile.getName())
                              ...
                              .withStyle(s -> s.withClickEvent(new ClickEvent.OpenFile(outputFile.getAbsoluteFile())));
