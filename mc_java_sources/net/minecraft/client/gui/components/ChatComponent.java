...
public class ChatComponent {
   ...
   private void extractRenderState(final ChatComponent.ChatGraphicsAccess graphics, int screenHeight, int ticks, ChatComponent.DisplayMode displayMode) {
      ...
      if (total > 0 || isRestricted) {
         ...
         final int entryHeight = (int)(9.0 * (chatLineSpacing + 1.0));
         ...
         if (queueSize > 0L) {
            graphics.fill(-2, chatBottom, maxWidth + 4, chatBottom + 9, ARGB.black(backgroundOpacity));
         }
         ...
         if (queueSize > 0L) {
            int queueLineBottom = chatBottom + 9;
