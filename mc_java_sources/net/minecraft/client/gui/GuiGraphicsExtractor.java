...
import net.minecraftforge.client.extensions.IForgeGuiGraphicsExtractor;
...
@OnlyIn(Dist.CLIENT)
public class GuiGraphicsExtractor implements IForgeGuiGraphicsExtractor {
   ...
   public void blit(
      RenderPipeline renderPipeline, Identifier texture, int x, int y, float u, float v, int width, int height, int textureWidth, int textureHeight, int color
   ) {
   ...
   private void item(@Nullable LivingEntity owner, @Nullable Level level, ItemStack itemStack, int x, int y, int seed) {
      if (!itemStack.isEmpty()) {
         ...
         try {
            ...
            category.setDetail("Registry Name", () -> String.valueOf(ForgeRegistries.ITEMS.getKey(itemStack.getItem())));
   ...
   public void setTooltipForNextFrame(Font font, List<Component> texts, Optional<TooltipComponent> optionalImage, int xo, int yo, @Nullable Identifier style) {
      List<ClientTooltipComponent> components = ForgeHooksClient.gatherTooltipComponents(
         this.tooltipStack, texts, optionalImage, xo, this.guiWidth(), this.guiHeight(), font
      );
   ...
   public void setComponentTooltipForNextFrame(Font font, List<Component> lines, int xo, int yo) {
      List<ClientTooltipComponent> components = ForgeHooksClient.gatherTooltipComponents(this.tooltipStack, lines, xo, this.guiWidth(), this.guiHeight(), font);
      this.setTooltipForNextFrameInternal(font, components, xo, yo, DefaultTooltipPositioner.INSTANCE, null, false);
   }

   public void renderComponentTooltip(Font font, List<? extends FormattedText> lines, int xo, int yo, ItemStack stack) {
      this.tooltipStack = stack;
      List<ClientTooltipComponent> components = ForgeHooksClient.gatherTooltipComponents(stack, lines, xo, this.guiWidth(), this.guiHeight(), font);
      this.setTooltipForNextFrameInternal(font, components, xo, yo, DefaultTooltipPositioner.INSTANCE, null, false);
      this.tooltipStack = ItemStack.EMPTY;
   }

   public void renderComponentTooltipFromElements(Font font, List<Either<FormattedText, TooltipComponent>> lines, int xo, int yo, ItemStack stack) {
      this.tooltipStack = stack;
      List<ClientTooltipComponent> components = ForgeHooksClient.gatherTooltipComponentsFromElements(stack, lines, xo, this.guiWidth(), this.guiHeight(), font);
      this.setTooltipForNextFrameInternal(font, components, xo, yo, DefaultTooltipPositioner.INSTANCE, null, false);
      this.tooltipStack = ItemStack.EMPTY;
   }
   ...
   private void setTooltipForNextFrameInternal(
      ...
      if (!lines.isEmpty() && (this.deferredTooltip == null || replaceExisting)) {
         ItemStack stackContext = this.tooltipStack;
   ...
   public void tooltip(
      Font font, List<ClientTooltipComponent> lines, int xo, int yo, ClientTooltipPositioner positioner, @Nullable Identifier style, ItemStack itemstack
   ) {
      RenderTooltipEvent.Pre preEvent = ForgeHooksClient.onRenderTooltipPre(
         itemstack, this, xo, yo, this.guiWidth(), this.guiHeight(), lines, font, positioner, style
      );
      if (preEvent != null) {
         int textWidth = 0;
         ...
         int w = textWidth;
         int h = tempHeight;
         Vector2ic positionedTooltip = positioner.positionTooltip(this.guiWidth(), this.guiHeight(), preEvent.getX(), preEvent.getY(), w, h);
         int x = positionedTooltip.x();
         int y = positionedTooltip.y();
         this.pose.pushMatrix();
         TooltipRenderUtil.extractTooltipBackground(this, x, y, w, h, preEvent.getBackground());
         int localY = y;
