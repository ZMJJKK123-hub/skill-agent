...
public class TitleScreen extends Screen {
   ...
   protected void init() {
      ...
      if (this.minecraft.isDemo()) {
         ...
         modButton = this.addRenderableWidget(
            Button.builder(Component.translatable("fml.menu.mods"), button -> this.minecraft.gui.setScreen(new ModListScreen(this)))
               .pos(this.width / 2 - 100, topPos)
               .size(98, 20)
               .build()
         );
      }
   ...
   private int createNormalMenuOptions(int topPos, int spacing) {
      ...
      if (SharedConstants.IS_RUNNING_IN_IDE) {
         this.addRenderableWidget(
            Button.builder(Component.literal("TW"), var1x -> CreateWorldScreen.testWorld(this.minecraft, () -> this.minecraft.gui.setScreen(this)))
      ...
      this.addRenderableWidget(
            Button.builder(Component.translatable("menu.online"), var1x -> this.minecraft.gui.setScreen(new RealmsMainScreen(this)))
               .bounds(this.width / 2 + 2, topPos = var7 + spacing, 98, 20)
               .tooltip(tooltip)
               .build()
         )
   ...
   public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
      ...
      float widgetFade_f = widgetFade;
      BrandingControl.forEachLine(
         true, true, (brd, brdline) -> graphics.text(this.font, brd, 2, this.height - (10 + brdline * (9 + 1)), ARGB.color(widgetFade_f, -1))
      );
      BrandingControl.forEachAboveCopyrightLine(
         (brd, brdline) -> graphics.text(
            this.font, brd, this.width - this.font.width(brd), this.height - (10 + (brdline + 1) * (9 + 1)), ARGB.color(widgetFade_f, -1)
         )
      );
      if (this.realmsNotificationsEnabled() && widgetFade >= 1.0F) {
         ...
         if (widgetFade >= 1.0F) {
            this.modUpdateNotification.extractRenderState(graphics, mouseX, mouseY, a);
         }
      }
   }
