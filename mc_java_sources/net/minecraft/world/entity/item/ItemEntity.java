...
public class ItemEntity extends Entity implements TraceableEntity {
   ...
   public void tick() {
      if (!this.getItem().onEntityItemUpdate(this)) {
         if (this.getItem().isEmpty()) {
            ...
            } else if (this.isInWater() && this.getFluidHeight(FluidTags.WATER) > 0.1F) {
            ...
            if (this.onGround() && !(this.getDeltaMovement().horizontalDistanceSqr() > 1.0E-5F) && (this.tickCount + this.getId()) % 4 != 0) {
               ...
               if (this.onGround()) {
                  ...
                  groundFriction *= this.level().getBlockState(groundPos).getFriction(this.level(), groundPos, this) * 0.98F;
               }
   ...
   protected void readAdditionalSaveData(ValueInput input) {
      ...
      this.lifespan = input.getIntOr("Lifespan", this.lifespan);
   ...
   public void playerTouch(Player player) {
      if (!this.level().isClientSide()) {
         ...
         if (this.pickupDelay == 0
            ...
            && (hook == 1 || orgCount <= 0 || player.getInventory().add(itemStack))) {
            orgCount = copy.getCount() - itemStack.getCount();
            copy.setCount(orgCount);
            ForgeEventFactory.firePlayerItemPickupEvent(player, this, copy);
   ...
   public void makeFakeItem() {
      this.setNeverPickUp();
      this.age = this.getItem().getEntityLifespan(this.level()) - 1;
   }
