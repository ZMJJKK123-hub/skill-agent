...
public class LookControl implements Control {
   ...
   protected Optional<Float> getXRotD() {
      ...
      return !(Math.abs(yd) > 1.0E-5F) && !(Math.abs(sd) > 1.0E-5F) ? Optional.empty() : Optional.of((float)(-(Mth.atan2(yd, sd) * 180.0 / (float) Math.PI)));
   }
   ...
   protected Optional<Float> getYRotD() {
      ...
      return !(Math.abs(zd) > 1.0E-5F) && !(Math.abs(xd) > 1.0E-5F)
         ? Optional.empty()
         : Optional.of((float)(Mth.atan2(zd, xd) * 180.0 / (float) Math.PI) - 90.0F);
   }
}
