...
public class EnderDragon extends Mob implements Enemy {
   ...
   private @Nullable EntityReference<Player> unlimitedLastHurtByPlayer = null;
         ...
         for (int y = y0; y <= y1; y++) {
            for (int z = z0; z <= z1; z++) {
               ...
               if (!state.isAir() && !state.is(BlockTags.DRAGON_TRANSPARENT)) {
                  if (ForgeHooks.canEntityDestroy(level, blockPos, this) && !state.is(BlockTags.DRAGON_IMMUNE)) {
   ...
   protected void tickDeath() {
      ...
      if (this.level() instanceof ServerLevel level) {
         if (this.dragonDeathTime > 150 && this.dragonDeathTime % 5 == 0 && level.getGameRules().get(GameRules.MOB_DROPS)) {
            int award = ForgeEventFactory.getExperienceDrop(this, this.getUnlimitedLastHurtByPlayer(), Mth.floor(xpCount * 0.08F));
      ...
      if (this.dragonDeathTime >= 200 && this.level() instanceof ServerLevel level) {
         if (level.getGameRules().get(GameRules.MOB_DROPS)) {
            int award = ForgeEventFactory.getExperienceDrop(this, this.getUnlimitedLastHurtByPlayer(), Mth.floor(xpCount * 0.2F));
            ExperienceOrb.award(level, this.position(), award);
         }
