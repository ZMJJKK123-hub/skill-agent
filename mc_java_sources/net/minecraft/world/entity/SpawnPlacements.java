...
public class SpawnPlacements {
   ...
   public static <T extends Entity> boolean checkSpawnRules(
      ...
      SpawnPlacements.Data data = DATA_BY_TYPE.get(type);
      boolean vanillaResult = data == null || data.predicate.test(type, level, spawnReason, pos, random);
      return ForgeEventFactory.checkSpawnPlacements(type, level, spawnReason, pos, random, vanillaResult);
   }
   ...
   public static void fireSpawnPlacementEvent() {
      ...
      DATA_BY_TYPE.forEach(
         (type, data) -> map.put((EntityType<?>)type, new SpawnPlacementRegisterEvent.MergedSpawnPredicate<>(data.predicate, data.placement, data.heightMap))
      );
      SpawnPlacementRegisterEvent.BUS.post(new SpawnPlacementRegisterEvent(map));
      map.forEach(
         (entityType, merged) -> DATA_BY_TYPE.put(
            (EntityType<?>)entityType, new SpawnPlacements.Data(merged.getHeightmapType(), merged.getSpawnType(), merged.build())
         )
      );
   }
