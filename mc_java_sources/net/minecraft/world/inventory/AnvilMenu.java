...
public class AnvilMenu extends ItemCombinerMenu {
   ...
   protected void onTake(Player player, ItemStack carried) {
      ...
      float breakChance = ForgeEventFactory.onAnvilRepair(player, carried, this.inputSlots.getItem(0), this.inputSlots.getItem(1)).getBreakChance();
      ...
      this.access.execute((level, pos) -> {
         ...
         if (!player.hasInfiniteMaterials() && state.is(BlockTags.ANVIL) && player.getRandom().nextFloat() < breakChance) {
   ...
   public void createResult() {
      ...
      if (!input.isEmpty() && EnchantmentHelper.canStoreEnchantments(input)) {
         ...
         if (!ForgeHooks.onAnvilChange(this, input, addition, this.resultSlots, this.itemName, tax, this.player)) {
            return;
         }

         if (!addition.isEmpty()) {
            usingBook = addition.has(DataComponents.STORED_ENCHANTMENTS);
         ...
         if (usingBook && !result.isBookEnchantable(addition)) {
